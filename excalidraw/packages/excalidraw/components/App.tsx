...
class App extends React.Component<AppProps, AppState> {
  ...
  /** last pointer position on canvas only (scene coords) */
  lastCanvasPointerCoords: { x: number; y: number } | null = null;
  ...
  private onWindowMessage(event: MessageEvent) {
    ...
    if (data?.type === "excalidraw:resize") {
      const nextWidthRaw =
        typeof data.width === "number" ? Math.ceil(data.width) : null;
      const nextHeightRaw =
        typeof data.height === "number" ? Math.ceil(data.height) : null;
      const messageElementId =
        typeof data.id === "string" && data.id.length > 0 ? data.id : null;
      const source = event.source as Window | null;
      const tryUpdate = (elementId: string) => {
        const element = this.scene.getNonDeletedElementsMap().get(elementId);
        if (!element || !isIframeElement(element)) {
          return false;
        }
        const nextHeight = nextHeightRaw;
        const nextWidth = nextWidthRaw;
        const shouldUpdateHeight =
          nextHeight !== null && Math.abs(nextHeight - element.height) > 1;
        const shouldUpdateWidth =
          nextWidth !== null && Math.abs(nextWidth - element.width) > 1;
        if (shouldUpdateHeight || shouldUpdateWidth) {
          this.scene.mutateElement(
            element,
            {
              ...(shouldUpdateHeight ? { height: nextHeight as number } : {}),
              ...(shouldUpdateWidth ? { width: nextWidth as number } : {}),
            },
      ...
      if (messageElementId) {
        const didUpdate = tryUpdate(messageElementId);
        if (didUpdate) {
          return;
        }
      }
      ...
      for (const [id, iframe] of this.iFrameRefs.entries()) {
  ...
  public onMagicframeToolSelect = () => {
    ...
    if (selectedElements.length === 0) {
      ...
      this.setState({ selectedElementIds: { [frame.id]: true } });
    }
  };
